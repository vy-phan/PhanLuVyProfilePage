...
const Menu: React.FC = () => {
  ...
  return (
    <div 
      ...
      } text-white rounded-full px-4 sm:px-8 py-2 sm:py-4 flex flex-wrap justify-center items-center w-[95%] sm:w-auto sm:max-w-xl mx-auto overflow-hidden`}
    >
      {menuItems.map((item, index) => (
        <React.Fragment key={item.id}>
          <button
            onClick={() => scrollToSection(item.id)}
            className={`px-2 sm:px-4 py-1 mx-1 sm:mx-2 text-sm sm:text-base transition-all duration-300 whitespace-nowrap ${
          ...
          {index < menuItems.length - 1 && (
            <span className="text-gray-600 select-none hidden sm:inline-block">|</span>
          )}
        </React.Fragment>
      ))}
    </div>
